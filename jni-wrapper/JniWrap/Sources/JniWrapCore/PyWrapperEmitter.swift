...
public struct PyWrapperEmitter {
    ...
    public func classDef(for cls: ClassNode) -> ClassDef {
        ...
        // Bases differ for interfaces vs. classes. pyjnius does not have a
        // separate `JavaInterface` base — interfaces are projected as plain
        // `JavaClass` subclasses (`PythonJavaClass` is only needed for the
        // reverse direction, i.e. implementing a Java interface in Python).
        let bases: [Expr] = [.name(Name(id: "JavaClass"))]
        _ = cls.kind  // kept on AST for non-pyjnius consumers
