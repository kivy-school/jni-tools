...
```
.jar/.aar  ──→  java -jar java-ast-emitter.jar  ──→  JSON (stdout)  ──→  Swift (JniWrapCore)  ──→  .py/.pyi
                     ↑                                                         ↑
...
- [x] Add `swift-java` as a SwiftPM dependency in `JniWrap/Package.swift`
...
- [x] Add a new `--backend swift-java` flag to `JniWrap.swift` CLI
...
- Our current architecture: see `jni-wrapper/README.md`
