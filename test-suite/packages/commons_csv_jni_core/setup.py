...
try:
    ...
    raise SystemExit(
        "Cython>=3.0 is required to build commons-csv-jni-core. "
...
if _ANDROID:
    pass  # extra Android-specific tweaks can go here if needed


class _FilteredBuildPy(_build_py):
    """Keep only target-appropriate typing/cimport artifacts in wheels.

    Desktop: .so + .pxd + .pyi  (full IDE/type-checker support)
    Android: .so + .pxd          (no .pyi — saves space, unused at runtime)
    """
